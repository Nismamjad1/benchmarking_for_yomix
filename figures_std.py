...
# Load the CSVs
scanpy_wilcoxon = pd.read_csv("/home/nisma/benchmark/benchmarking_for_yomix/output/TCGA/benchmark_mcc_scores_TCGA_scanpy_wilcoxon_one-vs-rest.csv")
cosg = pd.read_csv("/home/nisma/benchmark/benchmarking_for_yomix/output/TCGA/benchmark_mcc_scores_TCGA_cosg.csv")
yomix = pd.read_csv("/home/nisma/benchmark/benchmarking_for_yomix/output/TCGA/yomix-Sheet1.csv")
scanpy_ttest = pd.read_csv("/home/nisma/benchmark/benchmarking_for_yomix/output/TCGA/benchmark_mcc_scores_TCGA_scanpy_t-test_one-vs-rest.csv")
# Add method labels
yomix["Method"] = "Yomix"
...
# Convert MCC-related columns to numeric
for col in merged.columns:
    if any(x in col for x in ["mcc", "precision", "recall", "f1"]):
        merged[col] = pd.to_numeric(merged[col], errors='coerce')

# Clean Benchmark names just in case
merged["Benchmark"] = merged["Benchmark"].str.lower().str.strip()

# Feature sizes to consider
feature_sizes = [1, 3, 10, 20]

# Plotting overall average MCC across all benchmarks per method
plt.figure(figsize=(10, 6))
...
plt.title("Overall MCC Scores Averaged Across All Label for TCGA")
plt.xlabel("Number of Features")
plt.ylabel("Mean MCC Score")
plt.xticks(feature_sizes)
plt.grid(True)
